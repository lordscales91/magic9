...
public class MagicUtils {
	...
	private static String getPayloadFilename(boolean isOtherApp,
			FirmwareVersion ver) {
		// Actually, the ropbin filenames are the same, only changes the base
		// URL
	...
	public static String getSoundhaxFilename(FirmwareVersion ver) {
		...
		return String.format("soundhax-%s-%s.m4a", ver.getRegion().toString()
				.toLowerCase(), model);
	}

	public static String getSoundhaxURL(FirmwareVersion ver) {
		return MagicConstants.SOUNDHAX_BASE_URL + getSoundhaxFilename(ver)
				+ "?raw=true";
	}
	...
	private static String getCTR210Name(FirmwareVersion ver) {
		...
		return getCTRName(MagicConstants.CTR_VERSION_210, ver.getRegion()
				.toLetter(), model);
	}
	 ...
	 * were copied successfully by comparing their checksums. It delegates the
	 * actual copy of files to {@link MagicUtils#copyFile(File, File)} which
	 * will actually compare the checksums
	 */
	...
	/**
	 * This is a wrapper for {@link FileUtils#copyFile(File, File)} which
	 * computes the checksums to ensure the copy was successful.
	 */
	public static void copyFile(File srcFile, File destFile) throws IOException {
		FileUtils.copyFile(srcFile, destFile);
		if(FileUtils.checksumCRC32(srcFile) != FileUtils
				.checksumCRC32(destFile)) {
			throw new IOException(
					"Copy possibly failed. Checksums do not match");
		}
	}

	/**
	 * This is actually just a wrapper for
	 * {@link FileUtils#cleanDirectory(File)}. The wrapper is here to allow an
	 * easy transition to another library if necessary.
	 */
	 ...
	 * @param filename
	 *            The entry name to extract
	 * @param destination
	 *            The location to extract the entry
	 * @throws IOException
	 */
	public static void extractFileFromZip(File zipfile, String filename,
		...
		try {
			...
			if(FileUtils.checksumCRC32(destination) != entry.getCrc()) {
				throw new IOException(
						"Extraction possibly failed. Checksums do not match");
			}
		} finally {
	 ...
	 *            The zip file to extract
	 * @param outputdir
	 *            The directory to extract the contents
	 * @throws IOException
	 */
	public static void extractZipFile(File zipfile, File outputdir)
			throws IOException {
		ZipFile zip = null;
		try {
			zip = new ZipFile(zipfile);
			...
			for(Map.Entry<File, Long> entry : origChecksums.entrySet()) {
				if(FileUtils.checksumCRC32(entry.getKey()) != entry.getValue()
						.longValue()) {
					throw new IOException(
							"Extraction possibliy failed. Checksums do not match");
				}
			}
	 ...
	 * 
	 * @param zipfile
	 *            The 7-Zip file to extract
	 * @param outputdir
	 *            The directory to extract the contents
	 * @throws IOException
	 */
	public static void extract7ZipFile(File zipfile, File outputdir)
			throws IOException {
		SevenZFile zip = null;
		try {
			...
			while(entries.hasNext()) {
				...
				if(entry.isDirectory()) {
					...
					// Little workaround to get a working inputstream for this
					// entry.
	 ...
	 * @throws IOException
	 */
	public static void extractFileFrom7Zip(File zipfile, String filename,
			File destination) throws IOException {
		SevenZFile zip = null;
		try {
			zip = new SevenZFile(zipfile);
			...
			while(entries.hasNext()) {
				...
				if(currEntry.getName().equalsIgnoreCase(filename)) {
					entry = currEntry;
					break; // Letting the loop to continue would cause to
							// extract a different entry
				}
			}
			if(entry == null) {
				throw new IOException("Entry not found: " + filename);
	...
	/**
	 * Computes the checksum of the given files and returns them in map using
	 * filenames as key.<br/>
	 * If the first argument is a directory the checksums of the files inside it
	 * will be computed. Nested directories will be ignored.
	 * 
	 * @param files
	 *            to be processed
	 ...
	 * @throws IOException
	 */
	public static Map<String, Long> computeChecksums(File... files)
			throws IOException {
	...
	public static void saveStreamToFile(InputStream in, File out)
			throws IOException {
	...
	/**
	 * Loads the specified image resource and sets it as {@link ImageIcon} for
	...
	/**
	 * This method extracts the hexdigest from the shaFile;
	 */
	...
	public static String getPreMessageForStep(HackingStep step) {
		String message = null;
		if(HackingStep.NAND_BACKUP.equals(step)) {
			message = "First follow the instructions in the guide. Then, press proceed";
		}
		return message;
	}
	...
	public static String getMessageForStep(HackingStep step) {
		...
		if(HackingStep.HOMEBREW_SOUNDHAX.equals(step)) {
			...
			message = "Now press confirm to verify the backup integrity";
		}
		return message;
	}
}